...
int main()
{
	...
	for (int k = 0; k < number_of_student; k++) 		//for loop for output and for storing grades in @D array
	{	
		float sumGrades = 0;			//initialzie sum of grades to 0
		...
		grades[k] = new int[number_of_test];				//setting number of tests as columns

               	for (int j = 0; j < number_of_test; j++)			//loop for storing grades in 2D array
                {
                        cout << "Enter grade: ";				//output for grade
                        cin >> grades[k][j];					//storing grade in 2D array
			cout << endl;
				
			sumGrades = sumGrades + grades[k][j];			//adding value to sum of grades
		}		
			       		
		average[k]= (sumGrades/number_of_test);				//calculating average when one row for student is done
	}
...
//etGrades(grades, number_of_student, average);		//calling functions
	dispAverage(average, number_of_student);

	 for (int p = 0; p < number_of_student; p++)		//deleting rows
	 {
		delete [] grades[p];
	 }
